/*
* * Assignment4 - OTP: opt_enc_d.c
* * CS 344 - 2019 Fall 
* * Taekyoung Kim - Due date: Dec. 6, 2019
* * kimtaeky@oregonstate.edu
* * Description: this program works OTP client and server.
* * It gets files and info from client, which is otp_enc.c.
* * Then, it encrypts the text using key file and send the encrypted
* * file to the client again. 
*/
...
//I reused this struct from my smallsh.c to reap children pids 
/************************
struct PidBGs
{
    int countPidBG;
...
struct childBG* initCBG() {
	//for 5 children and one more for garbage.
...
//char* encryptText(char* plaintxt, char* key, int fileLeng)
void encryptText(char* plaintxt, char* key, int fileLeng)
{
   ...
   for (i=0; i < fileLeng; i++){
      ...
      chaK = (chaK == 32) ? 26 : (chaK - 65);
       ...
       //add chaK into chaT and get the modulo
      chaT += chaK;
      chaT = chaT % 27; //divide 27 because it has space as well as alphabets.

       //convert back to ascii number for character
      chaT += 65;
       // if the number is 91, it needs to change to space, which ascii num is 32. 
       if(chaT == 91) chaT = 32;

       plaintxt[i] = (char)chaT;   
...
{
	//initPidBgs();  // for the background child process id.
	//signal(SIGINT, killBGPids);
   ...
   {    waitChild(childPS);
      ...
      if (childID == 0){ //childID == 0
              ...
              memset(buffer, '\0', 256);
			  /**********************************************************************
			  ** First Receive: checking the connection and get the length.
			  ** And sending response: Y or N
			  ***********************************************************************/
			charsRead = recv(establishedConnectionFD, buffer, 255, 0); // Read the client's message from the socket
             ...
             if(strncmp(buffer, "ENC", 3) != 0) { //first 3 characters of the buffer is not matching with 'ENC'
			  			  
			 // if(clientSignal != serverENC) {
				  serverRes = 'N';
				 ...
				 if (charsRead < 0) error("ERROR writing Y message to socket");
		    ...
		    **************************************************************/
            ...
            //memset(encryptFile, '\0', sizeof(encryptFile));
			encryptText(textFromClient, keyText, fileLen);
                  
		 /****************************************************
		  ** Send: encrypted text
		  ** It also use while loop sending the text.
		  *****************************************************/
			currentBf = 0;
            remainBf = fileLen;
